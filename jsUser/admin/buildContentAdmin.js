function buildHTMLPost(){
    ...
    queryPOST_json('model/buildContentAdmin.php',dataSend,function(res){
      
       if(res.buildmypost==0){
         var html = "<span style='margin:0 auto;'>Không còn bài viết nào !</span>";
       }else{
        var html = "";
    ...
    '<btn type="button" class="btn btn-info">Facebook</btn></a><button type="button" class="btn btn-danger btn-xoa-mypost"> Xóa</button>'+       
    '<button type="button" class="btn btn-success btn-deny-mypost">Ẩn bài viết</button></pre></p>'+ 
      '</div>'+
    ...
    $(".form-content-admin").html(html)
    });
    }
 
    //================Sự kiện khi click vào một button xóa=========
    $('.form-content-admin').on('click','.btn-xoa-mypost',function(){
...
//================Sự kiện khi click vào một button xóa=========
      $('.form-content-admin').on('click','.btn-deny-mypost',function(){
     
        var mabaiviet = $(this).parents('.item-post').attr("data-mabaiviet");
        
                dataSend = {event:'deny',mabaiviet:mabaiviet};
            
                queryPOST_json('model/buildContentAdmin.php',dataSend,function(res){
                       if(res.deny==1){
                           buildHTMLPost();
                       }else{
                           bootboxalert("Vui lòng thử lại sau","Thao tác thất bại",function(){});
                       }
                }); 
    });
